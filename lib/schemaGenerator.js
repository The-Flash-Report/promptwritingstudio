...
export function generateArticleSchema(config) {
  const {
    ...
    url,
    slug,
    datePublished = new Date().toISOString().split('T')[0], // callers should pass explicit datePublished for stable serialization
    dateModified = new Date().toISOString().split('T')[0],
    keywords = []
  } = config

  const resolvedUrl = url || (slug ? `https://promptwritingstudio.com/${slug.replace(/^\//, '')}` : undefined)
